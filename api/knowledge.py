...
# Промпты для разных типов документов (фокус на неустойке и ст. 333 ГК РФ)
PARSE_PROMPTS = {
    "court_decision": """Ты — система извлечения данных из судебных решений.
ВАЖНО: Фокусируйся ТОЛЬКО на вопросах неустойки, пени, штрафов и применения ст. 333 ГК РФ.

Проанализируй текст судебного решения и извлеки следующую информацию в формате JSON:

{
  "title": "краткое название с акцентом на неустойку (например: 'Взыскание неустойки по договору поставки - отказ в снижении по ст. 333')",
  "case_number": "номер дела (например: А40-12345/2024)",
  "court_name": "название суда",
  "decision_date": "дата решения в формате YYYY-MM-DD",
  "summary": "краткое содержание ТОЛЬКО в части неустойки (2-4 предложения): размер неустойки, заявлял ли ответчик о снижении, какие доводы приводил, как суд решил вопрос о применении ст. 333 ГК РФ",
  "key_points": [
    "вывод суда о соразмерности/несоразмерности неустойки",
    "какие доказательства оценивал суд",
    "ссылки на позиции ВС РФ по ст. 333",
    "аргументы за/против снижения"
  ],
  ...
  "penalty_rate": "ставка неустойки по договору (например: 0.1% в день)",
  "penalty_amount": "сумма неустойки (если указана)"
}

ИГНОРИРУЙ информацию, не связанную с неустойкой. Выделяй аргументы суда по ст. 333 ГК РФ.
...
ВАЖНО: Извлекай ТОЛЬКО позиции, касающиеся неустойки, пени, штрафов и ст. 333 ГК РФ.
...

{
  ...
  "summary": "какие разъяснения даёт постановление по вопросам неустойки и ст. 333 ГК РФ (2-4 предложения)",
  "key_points": [
    "разъяснение по критериям несоразмерности неустойки",
    "разъяснение по бремени доказывания",
    "разъяснение по пределам снижения",
    "разъяснение по процессуальным вопросам применения ст. 333"
  ],
  "relevant_paragraphs": ["п. 69", "п. 71", "п. 73", "п. 77"]
}

Выдели 5-10 ключевых разъяснений ТОЛЬКО по неустойке и ст. 333 ГК РФ.
Если постановление не содержит позиций по неустойке - укажи это в summary.
Верни ТОЛЬКО JSON без дополнительного текста.
...
ВАЖНО: Извлекай ТОЛЬКО позиции по неустойке, пени, штрафам и применению ст. 333 ГК РФ.
...

{
  ...
  "summary": "какие правовые позиции по неустойке и ст. 333 ГК РФ содержит обзор (2-4 предложения)",
  "key_points": [
    "правовая позиция по критериям снижения неустойки",
    "правовая позиция по доказыванию несоразмерности",
    "примеры из практики когда снижали/не снижали"
  ],
  "cases_mentioned": ["А40-xxx/2023", "А56-xxx/2022"]
}

Выдели 5-10 ключевых правовых позиций ТОЛЬКО по неустойке и ст. 333 ГК РФ.
Если обзор не содержит позиций по неустойке - укажи это в summary.
Верни ТОЛЬКО JSON без дополнительного текста.
...
ВАЖНО: Извлекай ТОЛЬКО тезисы и выводы по неустойке, пени и применению ст. 333 ГК РФ.
...

{
  ...
  "summary": "основные выводы статьи по вопросам неустойки и ст. 333 ГК РФ (2-4 предложения)",
  "key_points": [
    "научная позиция по критериям несоразмерности",
    "анализ судебной практики по ст. 333",
    "предложения по совершенствованию"
  ],
  "practical_conclusions": [
    "практическая рекомендация для истца",
    "практическая рекомендация для ответчика",
    "аргументы для суда"
  ]
}

Выдели ключевые тезисы ТОЛЬКО по неустойке и ст. 333 ГК РФ.
Если статья не связана с неустойкой - укажи это в summary.
Верни ТОЛЬКО JSON без дополнительного текста.

Текст статьи:
""",

    "ai_review": """Ты — система извлечения данных из аналитических обзоров.
ВАЖНО: Извлекай ТОЛЬКО выводы и рекомендации по неустойке и применению ст. 333 ГК РФ.

Проанализируй текст аналитического обзора и извлеки следующую информацию в формате JSON:

{
  "title": "название/тема обзора",
  "authors": "автор или источник (если указан)",
  "source": "платформа или сервис (например: ChatGPT, Claude, Legal AI)",
  "decision_date": "дата создания в формате YYYY-MM-DD (если известна)",
  "summary": "основные выводы обзора по неустойке и ст. 333 ГК РФ (2-4 предложения)",
  "key_points": [
    "аналитический вывод о практике применения ст. 333",
    "выявленные тенденции",
    "типичные ошибки сторон"
  ],
  "practical_conclusions": [
    "рекомендация для противодействия снижению неустойки",
    "рекомендация по доказыванию",
    "рекомендация по формулировкам"
  ]
}

Выдели ключевые выводы ТОЛЬКО по неустойке и ст. 333 ГК РФ.
Верни ТОЛЬКО JSON без дополнительного текста.
...
DOCUMENT_TYPE_NAMES = {
    ...
    "ai_review": "Аналитический обзор от ИИ"
}
